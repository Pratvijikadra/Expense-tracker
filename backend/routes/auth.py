...
# 3. Direct Delete Endpoint (Bina Prefix Ke)
...
# ):
#     # Plain text password jo user ne dala
#     entered_password = request_data.password.encode('utf-8')
#     # Database mein saved hashed password
#     hashed_password = current_user["password"].encode('utf-8') if isinstance(current_user["password"], str) else current_user["password"]

#     # Inline password verification bypass algorithm (bcrypt)
...
#     # User ka link dhundne ke liye dono keys safe-side rakh lete hain
#     user_email = current_user.get("email")
#     user_username = current_user.get("username")

#     try:
#         # Step A: User se related saara data chun-chun kar saaf karna
#         await expense_collection.delete_many({"user_email": user_email})
#         await goal_collection.delete_many({"user_email": user_email})
#         await transfer_collection.delete_many({"user_email": user_email})
#         await transaction_collection.delete_many({"user_email": user_email})
        
#         # Username tracking fallback (agar kuch collections mein email ki jagah username saved ho)
#         if user_username:
#             await expense_collection.delete_many({"username": user_username})
#             await goal_collection.delete_many({"username": user_username})

#         # Step B: Main users collection se profile delete karna
...
# @router.post("/api/account/delete")
# def delete_user_account(request_data: DeleteAccountRequest, request: Request):
#     # 1. Aapka purana function direct payload nikalega
#     payload = get_current_user_from_cookie(request)
#     if not payload:
#         raise HTTPException(status_code=401, detail="Session expired! please login.")
    
#     user_identifier = payload.get("sub")
#     current_user = user_collection.find_one({"email": user_identifier}) or user_collection.find_one({"username": user_identifier})
    
#     if not current_user:
#         raise HTTPException(status_code=404, detail="User not found!")

#     # 2.  JWT/Bcrypt Password Check (Sirf yeh 1 line crash proof hai)
#     # Agar database ka password string hai toh use .encode() karna zaroori hai
